...
std::vector<std::string>  MX240::ping(uint8_t ID_pose)
 {
   std::vector<std::string> rplys;
   uint16_t m_len = 3;
   char m_tx_buffer[10]; 
    ...
    for(int i=0;i<sizeof(m_tx_buffer);i++)
      ...
      printf ("\n");
      uart.sendUart(m_tx_buffer);

    // MX240::simpleWrite(m_tx_buffer);

   

   rplys.push_back("test rply");
